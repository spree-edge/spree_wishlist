RSpec.describe Spree::WishedItemsController, type: :controller do
  ...
  context '#create' do
    context 'with valid params' do
      it 'creates a new Spree::WishedItem' do
        expect {
      ...
      it 'assigns a newly created wished_product as @wished_product' do
        post :create, params: { wished_product: attributes }
        expect(assigns(:wished_product)).to be_a Spree::WishedItem
        expect(assigns(:wished_product)).to be_persisted
      end

      it 'redirects to the created wished_product' do
        post :create, params: { wished_product: attributes }
        expect(response).to redirect_to spree.wishlist_path(Spree::WishedItem.last.wishlist)
      end
      ...
      it 'does not save if wished product already exist in wishlist' do
        ...
        }.to change(Spree::WishedItem, :count).by(0)
      end
    end
